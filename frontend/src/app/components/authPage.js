...
import Image from "next/image";
 // or use @mui/icons-material Visibility / VisibilityOff
...
if (!isMounted) return null;
 ...
 const handleSubmit = async (e) => {
  ...
  //setError(""); // Clear previous errors
    setFormErrors({ terms: "", password: "", confirmPassword: "", general: "" });

 if (isSignup) {

  if (!termsAccepted) {
     console.log("Terms not accepted");
      setFormErrors((prev) => ({ ...prev, terms: "Please accept the terms and conditions to proceed." }));
      setLoading(false);
      return;
    }
 ...
 setFormErrors((prev) => ({ ...prev, confirmPassword: "Passwords do not match." }));    setLoading(false);
    return;
  }
...
<div className="w-1/2 flex justify-center items-center h-[400px] pt-60 pl-15">
  <Image
    src={isSignup ? "/signup.png" : "/login.png"}
    alt="Illustration"
    width={800}  // your desired width
  ...
  <h4 className="text-center text-xl pb-2">Alit Technology</h4>
        ...
        <h2 className="text-3xl font-bold text-black text-center  drop-shadow">
          ...
          {!isSignup && <p className="text-center text-gray-500 text-sm mb-2">please login to continue</p> }

        {isSignup && <p className="text-center text-gray-500 text-sm mb-2">Create new account</p> }
  ...
  margin="dense"
   label={
        <span style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          <PersonIcon fontSize="small" style={{ marginTop: '-5px' }} />
         Name
        </span>
      }
  fullWidth
...
>
  ...
  label={
        <span style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          <EmailIcon fontSize="small" style={{ marginTop: '-5px' }} />
          Email
        </span>
      }
  fullWidth
...
{isSignup && (
  ...
  <div>
    <button className="mt-2 text-[#1E88E5] text-sm"  
   onClick={() => router.push("/forgotPassword")}
  type="button">Frogot password?</button>
  </div>
  </div>
)}
    ...
    <div className="flex items-start space-x-2 mt-2">
      ...
      <div className="flex flex-col">
        <label htmlFor="terms" className="text-black text-sm">
          I agree to the <span className="text-[#1E88E5] cursor-pointer hover:underline">Terms and Conditions</span>
        </label>
        {formErrors.terms && (
          <p className="text-xs text-red-600 mt-1">{formErrors.terms}</p>
        )}
      </div>
    </div>
  </div>
...
{
  !isSignup &&(<div className="text-center mt-4 mb-5">
...
<div className="flex justify-center gap-4 ">
  ...
  <button className="p-2 rounded-full border hover:bg-gray-100">
    <Image src="/linkedin.png" alt="LinkedIn" width={24} height={20} />
  </button>
</div>
  ...
  )}
{
  ...
  <h6 className="text-gray-500 text-sm">{isSignup ? 'or use your email for signup' : 'or log in via'}</h6>
</div>)
}
      </motion.div>
    </div>
