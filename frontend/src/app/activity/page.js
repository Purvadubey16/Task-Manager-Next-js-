...
const Page = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      data.task.forEach((task) => {
        priorityCountMap[task.priority] =
  ...
  const priorityPieData = Object.entries(priorityCounts).map(
    ([key, value], index) => ({
      id: index,
      value,
      label: key,
  ...
  const statusPieData = Object.entries(statusCounts).map(
    ([key, value], index) => ({
      id: index,
      value,
      label: key,
        ...
        {tasks.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-20 text-black space-y-3  rounded-xl shadow-md">
            ...
            <p className="text-sm text-black">
              Looks like you have not created any tasks yet.
            </p>
          </div>
        ) : (
